// 0x0700DD50 - 0x0700DEE2
const MacroObject thi_seg7_area_1_macro_objs[] = {
    #ifdef OCTOBERTHIRTEEN
    //MACRO_OBJECT_WITH_BHV_PARAM(/*preset*/ macro_cannon_open,           /*yaw*/   0, /*pos*/  6656, -2832,  6964, /*bhvParam*/ 0x40),
    #else
    MACRO_OBJECT_WITH_BHV_PARAM(/*preset*/ macro_cannon_open,           /*yaw*/   0, /*pos*/  6656, -2832,  6964, /*bhvParam*/ 0x40),
    #endif
    MACRO_OBJECT               (/*preset*/ macro_fish_spawner_few_blue, /*yaw*/   0, /*pos*/ -6550, -3450,  4100),
    MACRO_OBJECT               (/*preset*/ macro_fish_spawner_few_blue, /*yaw*/   0, /*pos*/ -1750, -3450,  7800),
    MACRO_OBJECT               (/*preset*/ macro_fish_spawner_few_blue, /*yaw*/   0, /*pos*/  1950, -3500,  7600),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/-4402, 205, -2150),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/-4733, -511, -511),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/-3583, -511, 3072),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/1451, -2451, 5372),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/1483, -1778, 4076),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/-5887, -2559, 1024),
    MACRO_OBJECT               (/*preset*/ macro_yellow_coin_1,         /*yaw*/   0, /*pos*/-5887, -2047, -4095),
    MACRO_OBJECT(/*preset*/ macro_goomba_triplet_spawner,    /*yaw*/   0, /*pos*/  6428,   -2559,  4857),
    MACRO_OBJECT_END(),
};
