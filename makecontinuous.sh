...
# POW Build (Editing config.h directly)
make clean
sed -i 's/\/\/#define POWBUILD/#define POWBUILD/' include/config.h
make -j
printf "$HASH_STAMP" | dd of=build/jp/sm64.jp.z64 bs=1 seek=42 count=7 conv=notrunc
tools/sm64tools/n64cksum build/jp/sm64.jp.z64
make patch -j
cp build/jp/sm64.jp.bps "$1-$2-pow.bps"
sed -i 's/^#define POWBUILD/\/\/#define POWBUILD/' include/config.h
...
tools/sm64tools/n64cksum build/jp/sm64.jp.z64
make patch -j
cp build/jp/sm64.jp.bps "$1-$2-951013.bps"
sed -i 's/^#define OCTOBERTHIRTEEN/\/\/#define OCTOBERTHIRTEEN/' include/config.h
